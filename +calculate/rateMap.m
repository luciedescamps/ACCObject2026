...
%     'caFeature' 'spike' for spike rate maps, 'amplitude' for amplitude
%                   rate maps. default is amplitude.
%    =========================================================================
%
%  OUTPUT
%
...
function [map, edges] = rateMap(pos, z, varargin)
    ...
    if size(z, 2) == 2 && ~isequal(size(z, 1), size(pos, 1))
    ...
    defaultMinTime = 0;
    defaultType = 'linear';
    defaultPosIndices = ones(size(pos, 1), 1);
    defaultSpkIndices = ones(size(z, 1), 1);
    defaultGroupType = true;
    defaultLimits = [];
    defaultCaFeature = 'amplitude';
    ...
    checkType = @(x) helpers.isstring(x, 'spike', 'amplitude');
    ...
    addParameter(inp, 'caFeature', defaultCaFeature);

    parse(inp, pos, z, varargin{:});
    % get parsed results
    ...
    caFeature = inp.Results.caFeature;
    ...
    map(numMaps).ObsAccumulate = [];

%     if isempty(cellEventsT)
%         return
%     end
    ...
    for p = 1:numPosGroups
        ...
        for s = 1:numIter
            if isempty(time) || isempty(z)
                % sometimes time can be empty if all position samples are NaN
                if isempty(y)
            ...
            if isempty(y)
                ...
                if isempty(z)
                    ...
                    if strcmp(caFeature,'spike')
                        Nspikes = general.accumulate([x y], n, [nBinsX nBinsY]);
                    else
                        Nspikes = general.accumulate([x y], z, [nBinsX nBinsY]);
                end
                map(m).countRaw = Nspikes'; % how many times (# spikes) animal was at a specific bin
                map(m).timeRaw = time;
                ...
                % smooth maps based on gaussian kernel
                map(m).z = general.smooth(zRaw, smooth);
                map(m).time = general.smooth(map(m).timeRaw, smooth);
                map(m).count = general.smooth(map(m).countRaw, smooth);
                
                % smoothed rate map based on adaptive smoothing
%                 nBins = (abs(limits(1)) + limits(2))/binWidth(1);
%                 [spkPos,~] = helper.spikePositions(cellEventsT, posData);
%                 [smoothed, ~] = helper.adaptiveSmoothing(posData, spkPos, 'binwidth', binWidth(1), 'nbins', nBins, 'shape', 1);
%                 map(m).adptSmooth = smoothed.z;
            end
            ...
            if showBlanks
                ...
                map(m).adptSmooth(selected) = NaN;
                map(m).smoothTest(selected) = NaN;
