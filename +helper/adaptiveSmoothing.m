...
%
%  USAGE
%   [map, posPdf] = helper.adaptiveSmoothing(pos, spkPos, <options>)
...
%    'alphaValue'   scaling parameter. Default value is 5000. See original paper
%                   for the details.
...
% 
% function originally developed for BNT (see +analysis.mapAdaptiveSmooting and modified by Miguel Carvalho 2020
% modifications: added parameter 'shape'. adadpted analysis for circular environment

function [smoothed, posPdf] = adaptiveSmoothing(pos, spkPos, varargin)
    % Check number of parameters
    if nargin < 2 || mod(length(varargin), 2) ~= 0,
        error('BNT:numArgs', 'Incorrect number of parameters (type ''help <a href="matlab:help helper.adaptiveSmoothing">helper.adaptiveSmoothing</a>'' for details).');
    end

    % Check parameter sizes
    if size(pos, 2) < 2
        error('Parameter ''pos'' should have at least 2 columns (type ''help <a href="matlab:help helper.adaptiveSmoothing">helper.adaptiveSmoothing</a>'' for details).');
    end
    if size(spkPos, 2) < 2
        error('Parameter ''spkPos'' should have 1 or 2 columns (type ''help <a href="matlab:help helper.adaptiveSmoothing">helper.adaptiveSmoothing</a>'' for details).');
    end
        ...
        switch(lower(varargin{i})),
            case 'binwidth',
                ...
                if ~helpers.isdscalar(binWidth, '>0')
                    error('Incorrect value for property ''binwidth'' (type ''help <a href="matlab:help analyses.mapAdaptiveSmoothing">analyses.mapAdaptiveSmoothing</a>'' for details).');
                end
                haveBinWidth = true;
                i = i + 2;
            ...
            case 'shape'
                shape = varargin{i+1};
                if ~(shape == 1 || shape == 2)
                    error('Incorrect value for property ''shape'' (type ''help <a href="matlab:help analyses.mapAdaptiveSmoothing">analyses.mapAdaptiveSmoothing</a>'' for details).');
                end
                i = i + 2;
        ...
        for ii = 1:numColBins
            ...
            for jj = 1:numRowBins
                ...
                if currentPosition > shape(2)/2
                    ...
                    % Set the rate for this bin
                    smoothed.z(jj,ii) = s/(n*sampleTime);
